...
Build before first use: `go build -o amd ./cmd/cli`

Requires same env vars as the API server (`ADVANCEDMD_USERNAME`, `ADVANCEDMD_PASSWORD`, `ADVANCEDMD_OFFICE_KEY`, `ADVANCEDMD_APP_NAME`, `REDIS_URL`, `API_SECRET`). All output is JSON to stdout; logs to stderr.
